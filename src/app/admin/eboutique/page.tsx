...
// ============================================================
// /admin/eboutique — gestion des macarons & du carrousel
// ============================================================

type Tab = 'badges' | 'carrousel'

export default function EboutiquePage() {
  const [authReady, setAuthReady] = useState(false)
  const [hasAccess, setHasAccess] = useState(false)
  const [currentUser, setCurrentUser] = useState<any>(null)
  const [tab, setTab] = useState<Tab>('badges')

  // ── Vérif auth + permission ──────────────────────────────
  useEffect(() => {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#0d0a08', fontFamily: "'DM Sans', system-ui, sans-serif", color: '#e8e0d5' }}>
      ...
      <main style={{ marginLeft: 220, flex: 1, padding: '32px 36px', maxWidth: 'calc(100vw - 220px)' }}>
        <h1 style={{ fontFamily: 'Georgia, serif', fontSize: 28, fontWeight: 300, color: '#f0e8d8', marginBottom: 4 }}>E-boutique</h1>
        <p style={{ fontSize: 12, color: 'rgba(232,224,213,0.35)', marginBottom: 24 }}>
          Macarons sur les fiches produits & carrousel de la page d'accueil
        </p>

        {/* Onglets */}
        <div style={{ display: 'flex', gap: 4, marginBottom: 28, borderBottom: '0.5px solid rgba(255,255,255,0.08)' }}>
          {(['badges', 'carrousel'] as Tab[]).map(t => (
            <button key={t} onClick={() => setTab(t)} style={{
              background: 'transparent', border: 'none', color: tab === t ? '#c9a96e' : 'rgba(232,224,213,0.45)',
              padding: '12px 20px', fontSize: 13, cursor: 'pointer', letterSpacing: 0.5,
              borderBottom: `2px solid ${tab === t ? '#c9a96e' : 'transparent'}`, marginBottom: -1,
            }}>
              {t === 'badges' ? 'Badges & macarons' : 'Carrousel d\'accueil'}
            </button>
          ))}
        </div>

        {tab === 'badges' ? <BadgesTab /> : <CarrouselTab />}
      </main>
    </div>
  )
}
...
function BadgesTab() {
  ...
  return (
    <div>
      ...
      {/* Section attribution badges → produits */}
      <section style={{ ...card, marginTop: 24 }}>
        <div style={cardHeader}>
          <h2 style={cardTitle}>Attribuer les badges aux produits</h2>
          <input
            type="text" placeholder="🔍 Filtrer un produit…" value={search}
      ...
      {(showCreate || editing) && (
        <BadgeEditor
          badge={editing || { mode: 'genere', actif: true, ordre: (badges.length + 1) }}
...
// ============================================================
// Onglet CARROUSEL D'ACCUEIL
// ============================================================
function CarrouselTab() {
  ...
  const [editing, setEditing] = useState<any | null>(null)
  const [showCreate, setShowCreate] = useState(false)
  const [managingProducts, setManagingProducts] = useState<any | null>(null) // slide
  ...
  return (
    <div>
      ...
      {(showCreate || editing) && (
        <SlideEditor
          slide={editing || { actif: true, ordre: slides.length + 1, position_texte: 'gauche', couleur_texte: '#ffffff', cta_label: 'Découvrir' }}
          onCancel={() => { setEditing(null); setShowCreate(false) }}
          onSave={handleSave}
        />
      )}
  ...
  return (
    <div style={modalOverlay} onClick={onCancel}>
      <div style={{ ...modalBox, maxWidth: 720 }} onClick={e => e.stopPropagation()}>
        ...
        <Field label="Titre *">
          <input type="text" value={s.titre || ''} onChange={e => setS({ ...s, titre: e.target.value })} style={input} placeholder="Ex : Opération rosé" />
        </Field>
        <Field label="Sous-titre (accroche)">
          <input type="text" value={s.sous_titre || ''} onChange={e => setS({ ...s, sous_titre: e.target.value })} style={input} placeholder="Jusqu'à -30% sur 100 références" />
        </Field>
        <Field label="Description (optionnel)">
          <textarea value={s.description || ''} onChange={e => setS({ ...s, description: e.target.value })} style={{ ...input, minHeight: 60, resize: 'vertical' as const }} />
        </Field>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
          <Field label="Fond (si pas d'image)">
            <input type="color" value={s.couleur_bg || '#8a6a3e'} onChange={e => setS({ ...s, couleur_bg: e.target.value })} style={{ ...input, height: 38, padding: 4 }} />
          </Field>
          <Field label="Couleur du texte">
            <input type="color" value={s.couleur_texte || '#ffffff'} onChange={e => setS({ ...s, couleur_texte: e.target.value })} style={{ ...input, height: 38, padding: 4 }} />
          </Field>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          <Field label="Slug (URL)">
            <input type="text" value={s.slug || ''} onChange={e => setS({ ...s, slug: e.target.value.toLowerCase().replace(/\s+/g, '-') })} style={input} placeholder="operation-rose" />
          </Field>
          <Field label="Label du bouton">
            <input type="text" value={s.cta_label || 'Découvrir'} onChange={e => setS({ ...s, cta_label: e.target.value })} style={input} />
          </Field>
        </div>

        <Field label="Lien personnalisé (sinon : /boutique?operation=<slug>)">
          <input type="text" value={s.cta_url_custom || ''} onChange={e => setS({ ...s, cta_url_custom: e.target.value })} style={input} placeholder="/boutique?couleur=rose" />
        </Field>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
          <Field label="Ordre">
            <input type="number" value={s.ordre ?? 0} onChange={e => setS({ ...s, ordre: parseInt(e.target.value) || 0 })} style={input} />
          </Field>
          <Field label="Visible à partir de">
            <input type="datetime-local" value={s.date_debut?.slice(0, 16) || ''} onChange={e => setS({ ...s, date_debut: e.target.value ? new Date(e.target.value).toISOString() : null })} style={input} />
          </Field>
          <Field label="Masquée à partir de">
            <input type="datetime-local" value={s.date_fin?.slice(0, 16) || ''} onChange={e => setS({ ...s, date_fin: e.target.value ? new Date(e.target.value).toISOString() : null })} style={input} />
          </Field>
        </div>
...
function ManageSlideProducts({ slide, allProducts, assocs, onClose }: {
  ...
  const handleSave = async () => {
    setSaving(true)
    try {
      // Stratégie simple : on supprime tout puis on réinsère (volume faible, OK)
...
// ── Preview d'une slide dans l'éditeur ──────────────────────
  ...
  return (
    <aside style={{ width: 220, background: '#100d0a', borderRight: '0.5px solid rgba(255,255,255,0.06)', display: 'flex', flexDirection: 'column' as const, padding: '24px 0', position: 'fixed' as const, top: 0, left: 0, bottom: 0, zIndex: 100 }}>
      ...
      <nav style={{ flex: 1, padding: '16px 0', overflowY: 'auto' as const }}>
        ...
        <div style={{ height: 12 }} />
        <a href="/admin/eboutique" style={{ ...navLink, color: '#c9a96e', borderLeft: '2px solid #c9a96e', background: 'rgba(201,169,110,0.08)' }}>
          <span>🛍️</span>E-boutique
        </a>
      </nav>
    </aside>
  )
...
const fullPage: any = { minHeight: '100vh', background: '#0d0a08', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(232,224,213,0.6)', flexDirection: 'column', gap: 12, fontFamily: "'DM Sans', system-ui, sans-serif" }
const card: any = { background: '#18130e', border: '0.5px solid rgba(255,255,255,0.07)', borderRadius: 6, overflow: 'hidden' }
const cardHeader: any = { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 18px', borderBottom: '0.5px solid rgba(255,255,255,0.05)' }
const cardTitle: any = { fontFamily: 'Georgia, serif', fontSize: 17, fontWeight: 300, color: '#f0e8d8', margin: 0 }
const empty: any = { padding: 60, textAlign: 'center', color: 'rgba(232,224,213,0.4)', fontSize: 13 }
const errBox: any = { marginBottom: 16, padding: '12px 16px', background: 'rgba(201,110,110,0.1)', border: '0.5px solid rgba(201,110,110,0.3)', borderRadius: 4, fontSize: 12, color: '#c96e6e' }
const th: any = { padding: '10px 12px', fontSize: 10, letterSpacing: 1.5, color: 'rgba(232,224,213,0.35)', fontWeight: 400, textTransform: 'uppercase' }
const td: any = { padding: '10px 12px', fontSize: 12, color: '#e8e0d5', verticalAlign: 'middle' }
const input: any = { width: '100%', background: 'rgba(255,255,255,0.04)', border: '0.5px solid rgba(255,255,255,0.15)', borderRadius: 4, color: '#e8e0d5', fontSize: 13, padding: '9px 12px', boxSizing: 'border-box' as const, fontFamily: 'inherit' }
const btnPrimary: any = { background: '#c9a96e', color: '#0d0a08', border: 'none', borderRadius: 4, padding: '9px 16px', fontSize: 12, letterSpacing: 0.5, fontWeight: 700, cursor: 'pointer' }
const btnGhost: any = { background: 'transparent', border: '0.5px solid rgba(255,255,255,0.15)', borderRadius: 4, padding: '9px 16px', fontSize: 12, color: 'rgba(232,224,213,0.7)', cursor: 'pointer' }
const btnGhostSmall: any = { background: 'transparent', border: '0.5px solid rgba(255,255,255,0.12)', borderRadius: 3, padding: '6px 10px', fontSize: 11, color: 'rgba(232,224,213,0.7)', cursor: 'pointer' }
const btnDangerSmall: any = { background: 'rgba(201,110,110,0.1)', border: '0.5px solid rgba(201,110,110,0.3)', borderRadius: 3, padding: '6px 10px', fontSize: 11, color: '#c96e6e', cursor: 'pointer' }
...
const modalOverlay: any = { position: 'fixed' as const, inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20, overflowY: 'auto' as const }
const modalBox: any = { background: '#18130e', border: '0.5px solid rgba(255,255,255,0.12)', borderRadius: 8, padding: 28, maxWidth: 520, width: '100%', maxHeight: 'calc(100vh - 40px)', overflowY: 'auto' as const }
const modalTitle: any = { fontFamily: 'Georgia, serif', fontSize: 20, fontWeight: 300, color: '#f0e8d8', marginTop: 0, marginBottom: 18 }
