...
import { ModuleRetourLocation } from '../../caisse/retour-location'
...
export default function LocationPage() {
  ...
  const genererBonCommande = (cmd: any, dateLiv?: string) => {
    ...
    const lignesHtml = lignes.map((l: any) =>
      ...
      '<td style="color:rgba(0,0,0,0.7)">' + l.fut?.contenance_litres + 'L</td>' +
      '<td style="text-align:center;font-weight:700;color:#8a6a3e">' + l.quantite + '</td>' +
      '<td style="text-align:right;color:rgba(0,0,0,0.75)">' + Number(l.prix_achat_ht).toFixed(2) + ' \u20ac</td>' +
      '<td style="text-align:right;font-weight:600;color:#0a0a0a">' + (l.prix_achat_ht * l.quantite).toFixed(2) + ' \u20ac</td></tr>'
    ).join('')
    ...
    return '<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><title>Bon de commande ' + cmd.numero + '</title>' +
      ...
      '.livraison-badge { display: inline-block; background: rgba(201,169,110,0.1); border: 0.5px solid rgba(201,169,110,0.4); border-radius: 4px; padding: 5px 12px; font-size: 12px; color: #8a6a3e; margin-top: 10px; }' +
      '.fournisseur { background: rgba(0,0,0,0.04); border-left: 3px solid rgba(201,169,110,0.4); padding: 12px 18px; margin-bottom: 32px; font-size: 13px; color: rgba(0,0,0,0.75); border-radius: 0 6px 6px 0; }' +
      '.fournisseur strong { color: #8a6a3e; }' +
      ...
      '.produit-nom { font-weight: 600; color: #0a0a0a; margin-bottom: 3px; }' +
      '.produit-type { font-size: 10px; color: rgba(0,0,0,0.45); text-transform: uppercase; letter-spacing: 1px; }' +
      ...
      '.total-line.sub { color: rgba(0,0,0,0.5); }' +
      '.total-line.tva { color: rgba(0,0,0,0.7); border-top: 0.5px solid rgba(0,0,0,0.07); }' +
      '.total-line.grand { background: rgba(201,169,110,0.08); border: 0.5px solid rgba(201,169,110,0.2); border-radius: 6px; margin: 12px 0 0; font-size: 18px; font-weight: 700; color: #8a6a3e; font-family: Georgia, serif; padding: 14px; }' +
      ...
      '.sig-title { color: rgba(138,106,62,0.85); font-size: 10px; letter-spacing: 1.5px; text-transform: uppercase; margin-bottom: 6px; }' +
      '.footer { margin-top: 40px; padding-top: 16px; border-top: 0.5px solid rgba(0,0,0,0.08); font-size: 10px; color: rgba(0,0,0,0.35); line-height: 2; }' +
          ...
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 8 }}>
            ...
            <button onClick={() => {
              ...
              const lignesFuts = (r.reservation_futs || []).map((rf: any) =>
                `<tr><td style="padding:10px 12px;border-bottom:0.5px solid rgba(255,255,255,0.06);font-size:13px;color:#1a1a1a">Location fût ${rf.fut?.nom_cuvee} ${rf.fut?.contenance_litres}L</td><td style="padding:10px 12px;border-bottom:0.5px solid rgba(255,255,255,0.06);text-align:center;font-size:13px;color:#1a1a1a">${rf.quantite}</td><td style="padding:10px 12px;border-bottom:0.5px solid rgba(255,255,255,0.06);text-align:right;font-size:13px;color:#8a6a3e">${fmt(rf.prix_unitaire_ttc * rf.quantite)}</td></tr>`
              ).join('')
              const lignesTireuses = (r.reservation_tireuses || []).map((rt: any) =>
                `<tr><td style="padding:10px 12px;border-bottom:0.5px solid rgba(255,255,255,0.06);font-size:13px;color:#1a1a1a">Tireuse : ${rt.tireuse?.nom} (${rt.tireuse?.modele})</td><td style="padding:10px 12px;border-bottom:0.5px solid rgba(255,255,255,0.06);text-align:center;font-size:13px;color:#1a1a1a">1</td><td style="padding:10px 12px;border-bottom:0.5px solid rgba(255,255,255,0.06);text-align:right;font-size:13px;color:#8a6a3e">incluse</td></tr>`
              ).join('')
              const html = `<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><title>Bon de confirmation ${r.numero}</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;background:#ffffff;color:#1a1a1a;max-width:800px;margin:0 auto;padding:40px 32px;-webkit-print-color-adjust:exact;print-color-adjust:exact}@media print{body{background:#ffffff!important}*{-webkit-print-color-adjust:exact!important}}.header{display:flex;justify-content:space-between;margin-bottom:32px;padding-bottom:20px;border-bottom:1px solid rgba(201,169,110,0.3)}.cave-name{font-size:20px;color:#8a6a3e;font-family:Georgia,serif;letter-spacing:2px}.cave-info{font-size:11px;color:rgba(0,0,0,0.5);line-height:2;margin-top:6px}.doc-title{font-size:11px;letter-spacing:4px;text-transform:uppercase;color:rgba(138,106,62,0.85);margin-bottom:6px;text-align:right}.doc-numero{font-size:22px;color:#8a6a3e;font-family:Georgia,serif;text-align:right}.section{background:rgba(0,0,0,0.04);border:0.5px solid rgba(0,0,0,0.1);border-radius:8px;padding:16px 20px;margin-bottom:16px}.section-title{font-size:10px;letter-spacing:2px;text-transform:uppercase;color:rgba(138,106,62,0.85);margin-bottom:10px}table{width:100%;border-collapse:collapse}th{padding:8px 12px;text-align:left;font-size:10px;letter-spacing:1.5px;text-transform:uppercase;color:rgba(138,106,62,0.85);font-weight:400;border-bottom:0.5px solid rgba(201,169,110,0.2)}.total-row{display:flex;justify-content:space-between;padding:10px 12px;font-size:18px;font-family:Georgia,serif;color:#8a6a3e;font-weight:700;background:rgba(201,169,110,0.06);border-radius:6px;margin-top:12px}.sig-zone{display:grid;grid-template-columns:1fr 1fr;gap:24px;margin-top:32px}.sig-box{border-top:0.5px solid rgba(201,169,110,0.3);padding-top:10px}.sig-label{font-size:10px;letter-spacing:1.5px;text-transform:uppercase;color:rgba(138,106,62,0.85);margin-bottom:32px}.footer{margin-top:28px;padding-top:14px;border-top:0.5px solid rgba(0,0,0,0.08);font-size:10px;color:rgba(0,0,0,0.35);line-height:2}</style></head><body>
              <div class="header"><div><div class="cave-name">Cave de Gilbert</div><div class="cave-info">Avenue Jean Colomb — 69280 Marcy l'Étoile<br>04 22 91 41 09 · contact@cavedegilbert.fr</div></div><div><div class="doc-title">Bon de confirmation</div><div class="doc-numero">${r.numero}</div><div style="font-size:12px;color:rgba(0,0,0,0.5);text-align:right;margin-top:4px">${new Date().toLocaleDateString('fr-FR')}</div></div></div>
              <div class="section"><div class="section-title">Client</div><div style="font-size:17px;color:#0a0a0a;font-family:Georgia,serif">${clientN}</div>${r.customer?.telephone ? `<div style="font-size:13px;color:rgba(0,0,0,0.6);margin-top:4px">📞 ${r.customer.telephone}</div>` : ''}</div>
              <div class="section"><div class="section-title">Période</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:16px"><div><div style="font-size:10px;color:rgba(0,0,0,0.5);letter-spacing:1px;margin-bottom:4px">DÉPART</div><div style="font-size:15px;color:#0a0a0a">${new Date(r.date_debut + 'T12:00:00').toLocaleDateString('fr-FR', {weekday:'long',day:'2-digit',month:'long',year:'numeric'})}</div></div><div><div style="font-size:10px;color:rgba(0,0,0,0.5);letter-spacing:1px;margin-bottom:4px">RETOUR</div><div style="font-size:15px;color:#0a0a0a">${new Date(r.date_fin + 'T12:00:00').toLocaleDateString('fr-FR', {weekday:'long',day:'2-digit',month:'long',year:'numeric'})}</div></div></div></div>
              <div class="section"><div class="section-title">Détail de la location</div><table><thead><tr><th>Désignation</th><th style="text-align:center">Qté</th><th style="text-align:right">Total TTC</th></tr></thead><tbody>${lignesTireuses}${lignesFuts}</tbody></table><div class="total-row"><span>Total TTC</span><span>${fmt(r.total_ttc || 0)}</span></div>${r.acompte_ttc ? `<div style="font-size:13px;color:#6ec96e;margin-top:8px;padding:8px 12px;background:rgba(110,201,110,0.08);border-radius:6px">✓ Acompte versé : ${fmt(r.acompte_ttc)} (${r.acompte_mode})<br><span style="color:rgba(0,0,0,0.6)">Solde à régler au retour : ${fmt((r.total_ttc||0) - r.acompte_ttc)}</span></div>` : ''}<div style="font-size:12px;color:rgba(0,0,0,0.5);margin-top:8px;padding:8px 12px;background:rgba(0,0,0,0.04);border-radius:6px">Caution tireuse : <strong style="color:#0a0a0a">${fmt(r.caution_tireuse_ttc||0)}</strong></div></div>
              <div class="sig-zone"><div class="sig-box"><div class="sig-label">Signature client — Bon pour accord</div></div><div class="sig-box"><div class="sig-label">Cave de Gilbert</div><div style="font-family:Georgia,serif;font-size:18px;color:#8a6a3e">Cave de Gilbert</div></div></div>
              <div class="footer">Cave de Gilbert · Avenue Jean Colomb, 69280 Marcy l'Étoile · contact@cavedegilbert.fr · 04 22 91 41 09</div></body></html>`
              const w = window.open('', '_blank'); if (w) { w.document.write(html); w.document.close(); w.print() }
            }} style={{ background: 'rgba(110,158,201,0.08)', border: '0.5px solid rgba(110,158,201,0.25)', borderRadius: 8, color: '#6e9ec9', padding: '12px 16px', fontSize: 13, cursor: 'pointer', textAlign: 'left' as const, display: 'flex', alignItems: 'center', gap: 10 }}>
              <span>📄</span><div><div style={{ fontSize: 13, color: '#6e9ec9' }}>Bon de confirmation</div><div style={{ fontSize: 11, color: 'rgba(232,224,213,0.35)', marginTop: 2 }}>Récapitulatif + signature client</div></div>
            </button>

            {/* Bon de livraison */}
            {['confirmée','en_cours','terminée'].includes(r.statut) && (
              <button onClick={() => {
                const clientN = r.customer?.est_societe ? r.customer.raison_sociale : `${r.customer?.prenom || ''} ${r.customer?.nom || ''}`.trim()
                const inventaire = ['Une tireuse froid sec','Un bec Easy Pracc',"L'outil pour retirer le bec Easy Pracc",'Un tuyau gris','Un tuyau rouge','Une tête de percussion','Une grille de récupération']
                const itemsHtml = inventaire.map(i => `<div style="display:flex;align-items:flex-start;gap:10px;padding:6px 0;border-bottom:0.5px solid rgba(255,255,255,0.04);font-size:14px;color:#1a1a1a"><span style="color:#8a6a3e;font-size:16px;flex-shrink:0">☑</span><span>${i}</span></div>`).join('')
                const futsHtml = (r.reservation_futs||[]).map((rf: any) => `<div style="display:flex;align-items:center;gap:10px;padding:6px 0;border-bottom:0.5px solid rgba(255,255,255,0.04);font-size:14px;color:#1a1a1a"><span style="color:#8a6a3e;font-size:16px">☑</span><span>${rf.quantite} × ${rf.fut?.nom_cuvee} ${rf.fut?.contenance_litres}L</span></div>`).join('')
                const tirsHtml = (r.reservation_tireuses||[]).map((rt: any) => `<div style="display:flex;align-items:center;gap:10px;padding:6px 0;border-bottom:0.5px solid rgba(255,255,255,0.04);font-size:14px;color:#1a1a1a"><span style="color:#8a6a3e;font-size:16px">☑</span><span>Tireuse : ${rt.tireuse?.nom} (${rt.tireuse?.modele})</span></div>`).join('')
                const html = `<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><title>Bon de livraison ${r.numero}</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;background:#ffffff;color:#1a1a1a;max-width:800px;margin:0 auto;padding:40px 32px;-webkit-print-color-adjust:exact;print-color-adjust:exact}@media print{body{background:#ffffff!important}*{-webkit-print-color-adjust:exact!important}}.header{display:flex;justify-content:space-between;margin-bottom:32px;padding-bottom:20px;border-bottom:1px solid rgba(201,169,110,0.3)}.cave-name{font-size:20px;color:#8a6a3e;font-family:Georgia,serif;letter-spacing:2px}.cave-info{font-size:11px;color:rgba(0,0,0,0.5);line-height:2;margin-top:6px}.section{background:rgba(0,0,0,0.04);border:0.5px solid rgba(0,0,0,0.1);border-radius:8px;padding:16px 20px;margin-bottom:16px}.section-title{font-size:10px;letter-spacing:2px;text-transform:uppercase;color:rgba(138,106,62,0.85);margin-bottom:10px}.warning{background:rgba(201,110,110,0.12);border:1px solid rgba(201,110,110,0.4);border-radius:8px;padding:14px 18px;margin:16px 0}.sig-zone{display:grid;grid-template-columns:1fr 1fr;gap:24px;margin-top:32px}.sig-box{border-top:0.5px solid rgba(201,169,110,0.3);padding-top:10px}.footer{margin-top:28px;padding-top:14px;border-top:0.5px solid rgba(0,0,0,0.08);font-size:10px;color:rgba(0,0,0,0.35);line-height:2}</style></head><body>
                <div class="header"><div><div class="cave-name">Cave de Gilbert</div><div class="cave-info">Avenue Jean Colomb — 69280 Marcy l'Étoile<br>04 22 91 41 09 · contact@cavedegilbert.fr</div></div><div><div style="font-size:11px;letter-spacing:4px;text-transform:uppercase;color:rgba(138,106,62,0.85);margin-bottom:6px;text-align:right">Bon de livraison</div><div style="font-size:22px;color:#8a6a3e;font-family:Georgia,serif;text-align:right">${r.numero}</div><div style="font-size:12px;color:rgba(0,0,0,0.5);text-align:right;margin-top:4px">${new Date().toLocaleDateString('fr-FR')}</div></div></div>
                <div class="section"><div class="section-title">Client</div><div style="font-size:17px;color:#0a0a0a;font-family:Georgia,serif">${clientN}</div>${r.customer?.telephone ? `<div style="font-size:13px;color:rgba(0,0,0,0.6);margin-top:4px">📞 ${r.customer.telephone}</div>` : ''}</div>
                <div class="section"><div class="section-title">Période</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:16px"><div><div style="font-size:10px;color:rgba(0,0,0,0.5);margin-bottom:4px">DÉPART</div><div style="font-size:15px;color:#0a0a0a">${new Date(r.date_debut + 'T12:00:00').toLocaleDateString('fr-FR', {weekday:'long',day:'2-digit',month:'long'})}</div></div><div><div style="font-size:10px;color:rgba(0,0,0,0.5);margin-bottom:4px">RETOUR</div><div style="font-size:15px;color:#0a0a0a">${new Date(r.date_fin + 'T12:00:00').toLocaleDateString('fr-FR', {weekday:'long',day:'2-digit',month:'long'})}</div></div></div></div>
                ${tirsHtml || futsHtml ? `<div class="section"><div class="section-title">Matériel remis</div>${tirsHtml}${futsHtml}</div>` : ''}
                <div class="section"><div class="section-title">Inventaire complet du matériel fourni</div>${itemsHtml}</div>
                <div class="warning"><div style="font-size:14px;color:#c96e6e;font-weight:700;letter-spacing:0.5px;text-transform:uppercase">⚠ Démontez absolument le bec Easy Pracc avant tout transport de la tireuse</div></div>
                <div class="sig-zone"><div class="sig-box"><div style="font-size:10px;letter-spacing:1.5px;text-transform:uppercase;color:rgba(138,106,62,0.85);margin-bottom:32px">Signature client — Bon pour accord</div></div><div class="sig-box"><div style="font-size:10px;letter-spacing:1.5px;text-transform:uppercase;color:rgba(138,106,62,0.85);margin-bottom:8px">Cave de Gilbert</div><div style="font-family:Georgia,serif;font-size:18px;color:#8a6a3e">Cave de Gilbert</div></div></div>
                <div class="footer">Cave de Gilbert · Avenue Jean Colomb, 69280 Marcy l'Étoile · contact@cavedegilbert.fr · 04 22 91 41 09</div></body></html>`
                const w = window.open('', '_blank'); if (w) { w.document.write(html); w.document.close(); w.print() }
              }} style={{ background: 'rgba(110,201,110,0.08)', border: '0.5px solid rgba(110,201,110,0.25)', borderRadius: 8, color: '#6ec96e', padding: '12px 16px', fontSize: 13, cursor: 'pointer', textAlign: 'left' as const, display: 'flex', alignItems: 'center', gap: 10 }}>
                <span>🚚</span><div><div style={{ fontSize: 13, color: '#6ec96e' }}>Bon de livraison</div><div style={{ fontSize: 11, color: 'rgba(232,224,213,0.35)', marginTop: 2 }}>Inventaire matériel + avertissement bec + signature</div></div>
              </button>
            )}
            ...
            {r.statut === 'terminée' && (
              <button onClick={() => {
                const clientN = r.customer?.est_societe ? r.customer.raison_sociale : `${r.customer?.prenom || ''} ${r.customer?.nom || ''}`.trim()
                const numeroFac = `FAC-${r.numero.replace('LOC-','')}`
                const tvaRate = 0.20
                const lignesHtml = (r.reservation_futs||[]).map((rf: any) => {
                  const prixTTC = rf.prix_unitaire_ttc || 0
                  const prixHT = prixTTC / (1 + tvaRate)
                  const totalHT = prixHT * rf.quantite
                  return `<tr><td style="padding:10px 12px;border-bottom:0.5px solid rgba(255,255,255,0.06);font-size:13px;color:#1a1a1a">Location fût ${rf.fut?.nom_cuvee} ${rf.fut?.contenance_litres}L<br><span style="font-size:10px;color:rgba(0,0,0,0.5)">${new Date(r.date_debut+'T12:00:00').toLocaleDateString('fr-FR')} au ${new Date(r.date_fin+'T12:00:00').toLocaleDateString('fr-FR')}</span></td><td style="padding:10px 12px;border-bottom:0.5px solid rgba(255,255,255,0.06);text-align:center;font-size:13px;color:#1a1a1a">${rf.quantite}</td><td style="padding:10px 12px;border-bottom:0.5px solid rgba(255,255,255,0.06);text-align:right;font-size:13px;color:rgba(0,0,0,0.7)">${prixHT.toFixed(2)} €</td><td style="padding:10px 12px;border-bottom:0.5px solid rgba(255,255,255,0.06);text-align:right;font-size:13px;color:#8a6a3e">${(prixTTC*rf.quantite).toFixed(2)} €</td></tr>`
                }).join('')
                const totalTTC = r.total_ttc || 0
                const totalHT = totalTTC / (1 + tvaRate)
                const tva = totalTTC - totalHT
                const html = `<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><title>Facture ${numeroFac}</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;background:#ffffff;color:#1a1a1a;max-width:800px;margin:0 auto;padding:40px 32px;-webkit-print-color-adjust:exact;print-color-adjust:exact}@media print{body{background:#ffffff!important}*{-webkit-print-color-adjust:exact!important}}.header{display:flex;justify-content:space-between;margin-bottom:32px;padding-bottom:20px;border-bottom:1px solid rgba(201,169,110,0.3)}.cave-name{font-size:20px;color:#8a6a3e;font-family:Georgia,serif;letter-spacing:2px}.cave-info{font-size:11px;color:rgba(0,0,0,0.5);line-height:2;margin-top:6px}.client-box{background:rgba(0,0,0,0.04);border-left:3px solid rgba(201,169,110,0.4);padding:12px 18px;margin-bottom:24px;font-size:13px;color:rgba(0,0,0,0.75);border-radius:0 6px 6px 0}table{width:100%;border-collapse:collapse}thead th{padding:8px 12px;text-align:left;font-size:9px;letter-spacing:2px;text-transform:uppercase;color:rgba(138,106,62,0.85);font-weight:400;border-bottom:1px solid rgba(201,169,110,0.3)}.totaux{border-top:1px solid rgba(201,169,110,0.2);margin-top:8px}.total-line{display:flex;justify-content:space-between;padding:8px 12px;font-size:13px;color:rgba(0,0,0,0.6)}.total-grand{display:flex;justify-content:space-between;padding:14px;background:rgba(201,169,110,0.08);border:0.5px solid rgba(201,169,110,0.2);border-radius:6px;margin-top:10px;font-size:20px;font-weight:700;color:#8a6a3e;font-family:Georgia,serif}.footer{margin-top:32px;padding-top:14px;border-top:0.5px solid rgba(0,0,0,0.08);font-size:10px;color:rgba(0,0,0,0.35);line-height:2}</style></head><body>
                <div class="header"><div><div class="cave-name">Cave de Gilbert</div><div class="cave-info">Avenue Jean Colomb — 69280 Marcy l'Étoile<br>04 22 91 41 09 · contact@cavedegilbert.fr</div></div><div><div style="font-size:11px;letter-spacing:4px;text-transform:uppercase;color:rgba(138,106,62,0.85);margin-bottom:6px;text-align:right">Facture</div><div style="font-size:22px;color:#8a6a3e;font-family:Georgia,serif;text-align:right">${numeroFac}</div><div style="font-size:12px;color:rgba(0,0,0,0.5);text-align:right;margin-top:4px">${new Date().toLocaleDateString('fr-FR')}</div></div></div>
                <div class="client-box"><strong style="color:#8a6a3e">Client :</strong> ${clientN}${r.customer?.telephone ? ` · 📞 ${r.customer.telephone}` : ''}${r.customer?.email ? `<br>${r.customer.email}` : ''}</div>
                <table><thead><tr><th>Désignation</th><th style="text-align:center">Qté</th><th style="text-align:right">P.U. HT</th><th style="text-align:right">Total TTC</th></tr></thead><tbody>${lignesHtml}</tbody></table>
                <div class="totaux"><div class="total-line"><span>Total HT</span><span>${totalHT.toFixed(2)} €</span></div><div class="total-line"><span>TVA 20 %</span><span>${tva.toFixed(2)} €</span></div>${r.acompte_ttc ? `<div class="total-line"><span>Acompte versé (${r.acompte_mode})</span><span>- ${fmt(r.acompte_ttc)}</span></div>` : ''}<div class="total-grand"><span>TOTAL TTC</span><span>${r.acompte_ttc ? fmt(totalTTC - r.acompte_ttc) + ' (solde)' : fmt(totalTTC)}</span></div></div>
                <div class="footer">Cave de Gilbert · Avenue Jean Colomb, 69280 Marcy l'Étoile · contact@cavedegilbert.fr · 04 22 91 41 09</div></body></html>`
                const w = window.open('', '_blank'); if (w) { w.document.write(html); w.document.close(); w.print() }
              }} style={{ background: 'rgba(201,169,110,0.08)', border: '0.5px solid rgba(201,169,110,0.25)', borderRadius: 8, color: '#c9a96e', padding: '12px 16px', fontSize: 13, cursor: 'pointer', textAlign: 'left' as const, display: 'flex', alignItems: 'center', gap: 10 }}>
                <span>💼</span><div><div style={{ fontSize: 13, color: '#c9a96e' }}>Facture finale</div><div style={{ fontSize: 11, color: 'rgba(232,224,213,0.35)', marginTop: 2 }}>Facture TTC avec déduction acompte</div></div>
              </button>
            )}
          </div>
        </div>
