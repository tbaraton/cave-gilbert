...
function Badge({ label, bg, color }: { label: string; bg: string; color: string }) {
  return (
    <span style={{ background: bg, color, fontSize: 10, fontWeight: 500, padding: '2px 8px', borderRadius: 3, letterSpacing: 1, textTransform: 'uppercase' as const, whiteSpace: 'nowrap' as const }}>
      {label}
    </span>
  )
}
...
function ModalEditClient({ client, onClose, onSaved }: { client: any; onClose: () => void; onSaved: () => void }) {
  ...
  const handleSave = async () => {
    setLoading(true)
    setError('')
    const { error: err } = await supabase.from('customers').update(form).eq('id', client.id || client.customer_id)
    ...
    <div style={{ position: 'fixed' as const, inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={onClose}>
      <div style={{ background: '#18130e', border: '0.5px solid rgba(201,169,110,0.2)', borderRadius: 8, width: 560, maxHeight: '88vh', overflowY: 'auto' as const, padding: '28px 32px' }} onClick={e => e.stopPropagation()}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <h2 style={{ fontFamily: 'Georgia, serif', fontSize: 20, fontWeight: 300, color: '#f0e8d8', margin: 0 }}>Modifier le client</h2>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <input value={form.adresse_ligne1} onChange={e => setForm(f => ({ ...f, adresse_ligne1: e.target.value }))} placeholder="Ligne 1"
          ...
          <input value={form.adresse_ligne2} onChange={e => setForm(f => ({ ...f, adresse_ligne2: e.target.value }))} placeholder="Ligne 2 (optionnel)"
            style={{ width: '100%', background: 'rgba(255,255,255,0.04)', border: '0.5px solid rgba(255,255,255,0.12)', borderRadius: 4, color: '#e8e0d5', fontSize: 13, padding: '9px 12px', boxSizing: 'border-box' as const }} />
        </div>
        ...
        <div style={{ marginBottom: 14 }}>
          <label style={{ fontSize: 10, letterSpacing: 1.5, color: 'rgba(232,224,213,0.4)', textTransform: 'uppercase' as const, display: 'block', marginBottom: 6 }}>Notes admin</label>
          <textarea value={form.notes_admin} onChange={e => setForm(f => ({ ...f, notes_admin: e.target.value }))} rows={3}
        ...
        <div style={{ marginBottom: 24, display: 'flex', alignItems: 'center', gap: 10 }}>
          <div onClick={() => setForm(f => ({ ...f, newsletter: !f.newsletter }))}
            style={{ width: 14, height: 14, borderRadius: 2, border: `0.5px solid ${form.newsletter ? '#c9a96e' : 'rgba(255,255,255,0.2)'}`, background: form.newsletter ? '#c9a96e' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
...
function ModalAjoutPoints({ client, onClose, onSaved }: { client: any; onClose: () => void; onSaved: () => void }) {
  ...
  const handleSave = async () => {
    ...
    await supabase.from('loyalty_points').insert({
      customer_id: client.id || client.customer_id,
  ...
  return (
    <div style={{ position: 'fixed' as const, inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={onClose}>
      <div style={{ background: '#18130e', border: '0.5px solid rgba(201,169,110,0.2)', borderRadius: 8, width: 400, padding: '28px 32px' }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ marginBottom: 14 }}>
          <label style={{ fontSize: 10, letterSpacing: 1.5, color: 'rgba(232,224,213,0.4)', textTransform: 'uppercase' as const, display: 'block', marginBottom: 6 }}>Points (+ ajout / - retrait)</label>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={onClose} style={{ flex: 1, background: 'transparent', border: '0.5px solid rgba(255,255,255,0.1)', color: 'rgba(232,224,213,0.4)', borderRadius: 4, padding: '10px', fontSize: 11, cursor: 'pointer' }}>Annuler</button>
...
export default function AdminClientsPage() {
  ...
  const loadBonsEnAttente = useCallback(async () => {
    const { data } = await supabase.from('v_vouchers_en_attente').select('*')
  ...
  const clientsFiltres = clients.filter(c => {
    ...
    return `${c.prenom || ''} ${c.nom || ''} ${c.email || ''}`.toLowerCase().includes(q)
  })

  const card = { background: '#18130e', border: '0.5px solid rgba(255,255,255,0.07)', borderRadius: 6, padding: '20px' } as React.CSSProperties
  const lbl = { fontSize: 10, letterSpacing: 1.5, color: 'rgba(232,224,213,0.35)', textTransform: 'uppercase' as const, display: 'block', marginBottom: 4 }

  const STATUT_V: Record<string, { bg: string; color: string }> = {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0d0a08', fontFamily: "'DM Sans', system-ui, sans-serif", color: '#e8e0d5', display: 'flex' }}>
      ...
      <main style={{ marginLeft: 220, flex: 1, padding: '32px 36px' }}>
        ...
        <div style={{ display: 'flex', gap: 0, marginBottom: 32, borderBottom: '0.5px solid rgba(255,255,255,0.08)' }}>
          ...
          {selectedClient && (
            <button onClick={() => setView('fiche')} style={{
              ...
              padding: '10px 20px', fontSize: 12, cursor: 'pointer', letterSpacing: 1, marginBottom: -1,
            }}>{selectedClient.prenom} {selectedClient.nom}</button>
          )}
        </div>
          ...
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              <div>
                ...
                <p style={{ fontSize: 12, color: 'rgba(232,224,213,0.35)' }}>{total} clients au total</p>
              </div>
            ...
            {loading ? <Spinner /> : (
              <>
                <div style={{ background: '#18130e', border: '0.5px solid rgba(255,255,255,0.07)', borderRadius: 6, overflow: 'hidden', marginBottom: 16 }}>
                    ...
                    <tbody>
                      {clientsFiltres.map((c, i) => (
                        <tr key={c.id || c.customer_id}
                          style={{ borderBottom: i < clientsFiltres.length - 1 ? '0.5px solid rgba(255,255,255,0.04)' : 'none', cursor: 'pointer' }}
                        ...
                        >
                          ...
                          <td style={{ padding: '12px 14px', fontSize: 13 }}>{c.nb_commandes || 0}</td>
                          ...
                          <td style={{ padding: '12px 14px' }}>
                            {(c.bons_en_attente || 0) > 0 && <Badge label={`${c.bons_en_attente} à valider`} bg="#2a2a1e" color="#c9b06e" />}
                            {(c.bons_valides || 0) > 0 && <Badge label={`${c.bons_valides} actif${c.bons_valides > 1 ? 's' : ''}`} bg="#1e2a1e" color="#6ec96e" />}
                          </td>
                          <td style={{ padding: '12px 14px' }}>
                            <button onClick={() => loadClientDetail(c)} style={{ background: 'transparent', border: '0.5px solid rgba(255,255,255,0.1)', color: 'rgba(232,224,213,0.4)', borderRadius: 3, padding: '5px 10px', fontSize: 10, cursor: 'pointer' }}>Voir →</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: 12, color: 'rgba(232,224,213,0.4)' }}>
                    {page * PAGE_SIZE + 1}–{Math.min((page + 1) * PAGE_SIZE, total)} sur {total} clients
                  </span>
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0} style={{
                      background: 'transparent', border: '0.5px solid rgba(255,255,255,0.15)',
                      color: 'rgba(232,224,213,0.5)', borderRadius: 4, padding: '8px 16px',
                      fontSize: 11, cursor: page === 0 ? 'not-allowed' : 'pointer', opacity: page === 0 ? 0.4 : 1,
                    }}>← Précédent</button>
        ...
        {view === 'bons' && (
          <>
            <h1 style={{ fontFamily: 'Georgia, serif', fontSize: 26, fontWeight: 300, color: '#f0e8d8', marginBottom: 24 }}>Bons d'achat à valider</h1>
            {bonsEnAttente.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 12 }}>
                {bonsEnAttente.map(bon => (
                  <div key={bon.id} style={{ ...card, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div>
                      ...
                      <div style={{ fontSize: 12, color: 'rgba(232,224,213,0.4)' }}>
                        ...
                        {' · '}Solde : <strong style={{ color: '#6ec96e' }}>{bon.points_actuels} pts</strong>
                        {' · '}{new Date(bon.genere_le).toLocaleDateString('fr-FR')}
                      </div>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <span style={{ fontFamily: 'Georgia, serif', fontSize: 22, color: '#c9a96e' }}>{bon.montant_ttc}€</span>
                      <button onClick={() => validateVoucher(bon.id)} style={{ background: '#1e2a1e', border: '0.5px solid rgba(110,201,110,0.3)', color: '#6ec96e', borderRadius: 4, padding: '8px 16px', fontSize: 11, cursor: 'pointer', letterSpacing: 1 }}>✓ Valider</button>
                      <button onClick={() => cancelVoucher(bon.id)} style={{ background: 'transparent', border: '0.5px solid rgba(255,255,255,0.1)', color: 'rgba(232,224,213,0.4)', borderRadius: 4, padding: '8px 12px', fontSize: 11, cursor: 'pointer' }}>Annuler</button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 28 }}>
              <div>
                ...
                <h1 style={{ fontFamily: 'Georgia, serif', fontSize: 26, fontWeight: 300, color: '#f0e8d8', marginBottom: 4 }}>{selectedClient.prenom} {selectedClient.nom}</h1>
                <p style={{ fontSize: 12, color: 'rgba(232,224,213,0.35)' }}>{selectedClient.email}</p>
              </div>
              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={() => setShowPointsModal(true)} style={{ background: 'transparent', border: '0.5px solid rgba(201,169,110,0.3)', color: '#c9a96e', borderRadius: 4, padding: '9px 16px', fontSize: 11, cursor: 'pointer', letterSpacing: 1 }}>+ Ajuster points</button>
                <button onClick={() => setShowEditModal(true)} style={{ background: '#c9a96e', color: '#0d0a08', border: 'none', borderRadius: 4, padding: '9px 16px', fontSize: 11, cursor: 'pointer', fontWeight: 500, letterSpacing: 1 }}>Modifier</button>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
              {[
                { label: 'Points fidélité', value: `${selectedClient.points_total || 0} pts`, color: (selectedClient.points_total || 0) >= 500 ? '#6ec96e' : '#c9a96e', sub: `${(selectedClient.points_total || 0) % 500} / 500 pts` },
                { label: 'Total achats', value: `${parseFloat(selectedClient.total_achats_ttc || 0).toFixed(2)}€`, color: '#c9a96e', sub: `${selectedClient.nb_commandes || 0} commande(s)` },
                { label: 'Bons actifs', value: selectedClient.bons_valides || 0, color: '#6ec96e', sub: `${selectedClient.bons_utilises || 0} utilisé(s)` },
                { label: 'À valider', value: selectedClient.bons_en_attente || 0, color: '#c9b06e', sub: 'En attente admin' },
            ...
            <div style={{ ...card, marginBottom: 24 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                <span style={lbl}>Progression vers le prochain bon (500 pts)</span>
                <span style={{ fontSize: 12, color: '#c9a96e' }}>{(selectedClient.points_total || 0) % 500} / 500</span>
              </div>
              <div style={{ height: 4, background: 'rgba(255,255,255,0.07)', borderRadius: 2 }}>
                <div style={{ width: `${Math.min(((selectedClient.points_total || 0) % 500) / 500 * 100, 100)}%`, height: '100%', background: 'linear-gradient(90deg, #c9a96e, #e8c98a)', borderRadius: 2 }} />
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
              ...
              <div style={card}>
                ...
                {[
                  ...
                  { label: 'Adresse', val: [selectedClient.adresse_ligne1, selectedClient.code_postal && selectedClient.ville ? `${selectedClient.code_postal} ${selectedClient.ville}` : null].filter(Boolean).join(', ') },
                ...
                {selectedClient.notes_admin && (
                  <div style={{ marginTop: 10, padding: '10px', background: 'rgba(255,255,255,0.03)', borderRadius: 4 }}>
                    <div style={lbl}>Notes admin</div>
                    ...
                    {clientVouchers.map(v => (
                      <div key={v.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 10px', background: 'rgba(255,255,255,0.03)', borderRadius: 4 }}>
                        <div>
                          ...
                          <div style={{ fontSize: 10, color: 'rgba(232,224,213,0.35)' }}>{new Date(v.genere_le).toLocaleDateString('fr-FR')} · {v.points_utilises} pts</div>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <span style={{ fontFamily: 'Georgia, serif', fontSize: 16, color: '#c9a96e' }}>{v.montant_ttc}€</span>
                          <Badge label={v.statut} bg={STATUT_V[v.statut]?.bg || '#222'} color={STATUT_V[v.statut]?.color || '#888'} />
                          {v.statut === 'en_attente' && (
              ...
              <div style={card}>
                ...
                {clientPoints.length === 0 ? (
                  <p style={{ color: 'rgba(232,224,213,0.3)', fontSize: 13 }}>Aucun mouvement.</p>
                ) : (
              ...
              <div style={card}>
                ...
                {clientHistory.length === 0 ? (
                  ...
                  <div style={{ maxHeight: 240, overflowY: 'auto' as const }}>
                    {clientHistory.map(o => (
                      <div key={o.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 10px', background: 'rgba(255,255,255,0.03)', borderRadius: 4, marginBottom: 6 }}>
                        <div>
                          <div style={{ fontFamily: 'monospace', fontSize: 12, color: '#c9a96e' }}>{o.numero}</div>
                          <div style={{ fontSize: 11, color: 'rgba(232,224,213,0.35)' }}>{new Date(o.created_at).toLocaleDateString('fr-FR')} · {o.order_items?.length || 0} article(s)</div>
                        </div>
