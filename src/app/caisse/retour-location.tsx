    ...
    try {
      ...
      for (const l of lignes) {
        ...
        if (l.quantite_non_percutee > 0) {
          const { data: futData } = await supabase.from('futs_catalogue').select('stock_actuel').eq('id', l.fut_catalogue_id).single()
          if (futData) await supabase.from('futs_catalogue').update({ stock_actuel: futData.stock_actuel + l.quantite_non_percutee }).eq('id', l.fut_catalogue_id)
        }
