...
export function genererFactureCaisseHtml(
  ...
  const lignesHtml = detail.notes
    ? `<tr><td colspan="4" style="padding:12px;color:#1a1a1a;font-size:13px">${detail.notes}</td><td style="padding:12px;text-align:right;color:#8a6a3e;font-size:14px;font-weight:700">${totalTTC.toFixed(2)} €</td></tr>`
    : lignesDetail.map((l) => {
        ...
        return `<tr style="border-bottom:0.5px solid rgba(0,0,0,0.08)">
          ...
          <td style="padding:10px 12px;text-align:right;font-size:13px;color:rgba(0,0,0,0.7)">${prixHT.toFixed(2)} €</td>
          <td style="padding:10px 12px;text-align:right;font-size:13px;color:rgba(0,0,0,0.5)">${(punit * tvaRate).toFixed(2)} €</td>
          <td style="padding:10px 12px;text-align:right;font-size:14px;color:#8a6a3e;font-weight:600">${ltot.toFixed(2)} €</td>
        </tr>`
      }).join('')
  ...
  const paiementsHtml = paiementsDetail.map((p) => {
    ...
    return `<div style="display:flex;justify-content:space-between;padding:6px 0;font-size:12px;color:rgba(0,0,0,0.6)">
      <span>${label}</span>
...
@media print { body { background: #ffffff !important; } * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; } }
...
.client-box { background: rgba(0,0,0,0.04); border-left: 3px solid rgba(201,169,110,0.4); padding: 12px 18px; margin-bottom: 32px; font-size: 13px; color: rgba(0,0,0,0.75); border-radius: 0 6px 6px 0; }
.client-box strong { color: #8a6a3e; font-size: 15px; }
table { width: 100%; border-collapse: collapse; }
thead tr { border-bottom: 1px solid rgba(201,169,110,0.3); }
thead th { padding: 10px 12px; text-align: left; font-size: 9px; letter-spacing: 2px; text-transform: uppercase; color: rgba(138,106,62,0.85); font-weight: 400; }
...
.total-grand { display: flex; justify-content: space-between; padding: 14px; background: rgba(201,169,110,0.08); border: 0.5px solid rgba(201,169,110,0.2); border-radius: 6px; margin-top: 10px; font-size: 22px; font-weight: 700; color: #8a6a3e; font-family: Georgia, serif; }
.rib-box { background: rgba(0,0,0,0.04); border: 0.5px solid rgba(0,0,0,0.1); border-radius: 8px; padding: 14px 18px; margin-top: 24px; }
.rib-title { font-size: 9px; letter-spacing: 2px; text-transform: uppercase; color: rgba(138,106,62,0.85); margin-bottom: 8px; }
.rib-line { font-size: 12px; color: rgba(0,0,0,0.6); line-height: 2; }
.rib-val { color: #0a0a0a; font-family: monospace; }
.footer { margin-top: 32px; padding-top: 14px; border-top: 0.5px solid rgba(0,0,0,0.08); font-size: 10px; color: rgba(0,0,0,0.4); line-height: 2; }
...
<div class="header">
  ...
  <div>
    ...
    <div class="doc-date" style="margin-top:2px;color:rgba(138,106,62,0.85)">Vendeur : ${detail.user?.prenom || ''}</div>
  </div>
</div>
...
<div style="background:rgba(110,158,201,0.06);border:0.5px solid rgba(110,158,201,0.2);border-radius:8px;padding:16px 20px;margin-top:24px">
  <div style="font-size:9px;letter-spacing:2px;text-transform:uppercase;color:rgba(58,94,138,0.85);margin-bottom:8px">Conditions d'acceptation</div>
  <div style="font-size:12px;color:rgba(0,0,0,0.75);line-height:2">
    Ce devis est valable <strong style="color:#1a1a1a">2 semaines</strong> à compter de la date d'émission, soit jusqu'au <strong style="color:#1a1a1a">${dateEcheance}</strong>.<br>
    Pour accepter ce devis, retournez-le signé avec la mention <em style="color:#8a6a3e">« Bon pour accord »</em>.<br>
    Les prix sont indiqués en TTC. TVA acquittée sur les débits. Paiement à la commande sauf accord préalable.
  </div>
</div>
<div style="margin-top:24px;border:0.5px solid rgba(0,0,0,0.15);border-radius:8px;overflow:hidden">
  <div style="display:grid;grid-template-columns:1fr 1fr 1fr">
    <div style="padding:14px 16px;border-right:0.5px solid rgba(0,0,0,0.1)">
      <div style="font-size:8px;letter-spacing:2px;text-transform:uppercase;color:rgba(0,0,0,0.45);margin-bottom:36px">Date d'acceptation</div>
      <div style="border-bottom:0.5px solid rgba(0,0,0,0.4)"></div>
    </div>
    <div style="padding:14px 16px;border-right:0.5px solid rgba(0,0,0,0.1)">
      <div style="font-size:8px;letter-spacing:2px;text-transform:uppercase;color:rgba(0,0,0,0.45);margin-bottom:36px">Bon pour accord</div>
      <div style="border-bottom:0.5px solid rgba(0,0,0,0.4)"></div>
    </div>
    <div style="padding:14px 16px">
      <div style="font-size:8px;letter-spacing:2px;text-transform:uppercase;color:rgba(0,0,0,0.45);margin-bottom:36px">Signature / Cachet</div>
      <div style="border-bottom:0.5px solid rgba(0,0,0,0.4)"></div>
    </div>
  </div>
</div>` : ''}
